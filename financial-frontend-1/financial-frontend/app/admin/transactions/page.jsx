...
export default function TransactionsPage() {
  const { fetchTransactions, transactions, loading, deleteTransaction } = useData()
  ...
  const handleDelete = async (transactionId) => {
    if (!confirm('Are you sure you want to delete this transaction? This will reverse the balance change and cannot be undone.')) {
      return
    }
    ...
    if (result.success) {
      setSuccess("Transaction deleted successfully! Balance has been reversed.")
  ...
  return (
    <div className="p-8 bg-slate-950 min-h-screen">
      ...
      <Card className="bg-slate-900 border-slate-800">
        <CardHeader>
          <CardTitle className="text-white">Recent Transactions ({transactions.length})</CardTitle>
          <CardDescription>Latest 100 transactions with management actions</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <tbody>
                  {transactions.slice(0, 50).map((txn) => (
                    <tr key={txn.id} className="border-b border-slate-800 hover:bg-slate-800">
                      ...
                      <td className="py-3 px-4 text-blue-400">₹{(txn.finalAmount / 1000).toFixed(2)}K</td>
